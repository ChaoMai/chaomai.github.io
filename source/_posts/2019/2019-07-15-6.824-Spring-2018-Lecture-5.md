...
* RequestVote RPCs：candidates调用，来进行拉票，用于选主。
...
* split votes（分裂投票）会很少出现。
    C先timeout，然后投票给自己，并发送RequestVote RPCs，最终win。
...
3. 当log entry被安全复制（成为*committed* log entry，leader更新commit index），
    ...
    * leader AppendEntries RPC通知follower committed entry。
    * follower apply log entry。
...
在选举的时候，使用voting process来避免未包含所有committed entry的candidate赢得选举。由于candidate在选举的时候需要联系大多数server，即只需要求每个committed entry必须出现在至少一个server里面即可。
...
因此不能仅通过副本数判断日志是不是committed，还需要满足*至少有一个属于当前term的log也复制到大多数server*，才能认为是。

## 问题
1. 日志复制时每个步骤都有可能出现错误，如何处理？
2. 为什么不能直接复制上一个term未committe的entry？
